 ...
 * 
 * Reusable social login button with proper styling and functionality
 */
...
const SocialLoginButton: React.FC<SocialLoginButtonProps> = ({
  ...
  const handleClick = async () => {
    ...
    try {
      switch (provider) {
        case 'google':
          if (socialAuth.isGoogleLoginEnabled()) {
            await socialAuth.signInWithGooglePopup();
          } else {
  ...
  const getProviderName = () => {
    switch (provider) {
      case 'google':
        return t('auth:social.google');
      case 'facebook':
        return t('auth:social.facebook');
      case 'apple':
        return t('auth:social.apple');
      default:
        return provider;
    }
  };
  ...
  const isProviderEnabled = () => {
    switch (provider) {
      case 'google':
        return socialAuth.isGoogleLoginEnabled();
      case 'facebook':
        return socialAuth.isFacebookLoginEnabled();
      case 'apple':
        return socialAuth.isAppleLoginEnabled();
      default:
        return false;
    }
  };
  ...
  return (
    <button
      ...
      className={`btn-social btn-${provider} ${className}`}
    >
      {isLoading ? (
